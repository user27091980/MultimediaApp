...
class UsersInfoRepo(context: Context) {
    ...
    suspend fun register(email: String, name: String, pass: String, lastName: String, country: String): Result<UsersInfoDTO> {
        return try {
            val dto = UsersInfoDTO("", email, pass, name, lastName, country)
    ...
    suspend fun update(dto: UsersInfoDTO): Result<UsersInfoDTO> {
        return try {
            ...
            } else Result.failure(Exception("No se pudo actualizar el perfil"))
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
    ...
    val loggedUser: Flow<UsersInfoDTO?> get() = session.userFlow
